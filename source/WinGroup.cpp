...
WindowSpec *WinGroup::IsMember(HWND aWnd, WindowSearchSettings const& aSettings)
{
	if (!aWnd)
		return NULL;  // Some callers on this.
	WindowSearch ws;
	ws.SetCandidate(aWnd);
	return IsMember(aWnd, aSettings, ws);
}



WindowSpec *WinGroup::IsMember(HWND aWnd, WindowSearchSettings const& aSettings, WindowSearch &ws, bool aInvert)
...
// thread-safety is maintained, even if the calling thread is the hook.
{
	...
	{
		if (ws.SetCriteria(aSettings, win->mTitle, win->mText, win->mExcludeTitle, win->mExcludeText) && ws.IsMatch(aInvert))
			return win;
...
// the linked list, and that hasn't already been visited.
{
	...
	if (ws.mSearchGroup->IsMember(aWnd, *g, ws, true))
		// We want to find non-members only, so keep searching:
		return TRUE;

	// aWnd is the one we're looking for unless it's in the list of exceptions:
...
// Caller must have ensured that lParam isn't NULL and that it contains a non-NULL mSearchGroup.
{
	WindowSearch &ws = *(WindowSearch *)lParam;  // For performance and convenience.
