...
{ // Added with the M2 search method (ImageSearch)
	...
	for (y1 = 0; y1 < sy2; y1++)
	{
		for (x1 = 0; x1 < sx2; x1++)
		{
			...
			if (abs(Needle[(4*suX)+(suY*Stride2)]- HayStack[(4*(x1 + suX))+((y1 + suY)*Stride1)]) <= v)
			{
				// Search direction 1
				// left -> right
				// top -> bottom
				if (sd == 1){
					...
					for (y2 = 0; y2 < nh; y2++)
					{
						...
						for (x2 = 0; x2 < nw; x2++)
						{
							if (abs(Needle[(4*x2)+(y2*Stride2)+2] - HayStack[(4*tx)+(ty*Stride1)+2]) <= v
							&& abs(Needle[(4*x2)+(y2*Stride2)+1] - HayStack[(4*tx)+(ty*Stride1)+1]) <= v
							&& abs(Needle[(4*x2)+(y2*Stride2)] - HayStack[(4*tx)+(ty*Stride1)]) <= v
							|| Needle[(4*x2)+(y2*Stride2)+3] == 0)
								tx++;
							else
								goto NoMatch;
